'use strict';
import {output} from './molecules_similarity.js'
...
//(output)=>{
    const graph1 = selectSubgraph(response.graph1, response.mcs);
    const graph2 = selectSubgraph(response.graph2, response.mcs);
    ...
    document.getElementById('similarity').innerHTML = response.similarity;
    document.getElementById('graph1_name').innerHTML = response.graph1.name.toUpperCase();
    document.getElementById('graph2_name').innerHTML = response.graph2.name.toUpperCase();
    setTimeout(()=>{
